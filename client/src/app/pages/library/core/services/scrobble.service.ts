...
export class ScrobbleService {
  ...
  private categoryScrobbles: BehaviorSubject<ICategoryScrobbles | null> = new BehaviorSubject<ICategoryScrobbles | null>(null);
  ...
  getTopArtists(period: any, forceReload = false): Observable<any> {
    if (!this.topArtists[period] || forceReload) {
      ...
      this.fetchArtistScrobbles(period).subscribe();
    }
    return this.topArtists[period].asObservable();
  }

  private fetchArtistScrobbles(period: any): any {
    return this.http
      .get<IScrobble[]>(`${SERVER_API_URL}/scrobbles/artist`, {
        params: {
          range: String(period),
        },
        observe: "response",
  ...
  getTopAlbums(period: any, forceReload = false): Observable<any> {
    if (!this.topAlbums[period] || forceReload) {
      ...
      this.fetchAlbumScrobbles(period).subscribe();
    }
    return this.topAlbums[period].asObservable();
  }

  private fetchAlbumScrobbles(period: any): any {
    return this.http
  ...
  getTopTracks(period: any, forceReload = false): Observable<any> {
    if (!this.topTracks[period] || forceReload) {      
      ...
      this.fetchTrackScrobbles(period).subscribe();
    }
    return this.topTracks[period].asObservable();
  }

  private fetchTrackScrobbles(period: any): any {
    return this.http
      .get<IScrobble[]>(`${SERVER_API_URL}/scrobbles/track`, {
        params: {
          range: String(period),
        },
        observe: "response",
